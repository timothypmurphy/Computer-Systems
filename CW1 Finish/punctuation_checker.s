...
IsAlpha:
    ...
    li $s6, 44 			#if(c == ',' || c == '.' || c == '!' || c == '?') go to PunctCheck
    ...
    li $s6, 32			#if(c == ' ') go to LoopTok
    beq $s4, $s6, LoopTok 	
    
    li $s6, 0			#if(c == ' ') go to LoopTok
    beq $s4, $s6, LoopTok 
...
#space before
    ...
    mflo $t7			 	# $t7 = $t7 * $s3;
    add $t7, $t1, $t7		  	# $t7 += $t1;
    lb $t6, 0($t7)			# $t6 = Tokens[$t7];
    li $t8, 32				# $t8 = 32;
    
    beq $t8, $t6, PunctIncorrect	# if($t8 == $t6) then go to PunctIncorrect
...
#Alpha after 
    ...
    li $t8, 44 				
    beq $t6, $t8, ContP1		# if ($t6 == $44 || $t6 == 46 || $t6 == 0 || $t6 == 33 || $t6 == 63 || $t6 == 10) then go to ContP1
    ...
    j PunctIncorrect			# else go to PunctIncorrect;
...
# is ellipsis
    ...
    bne $t8, $t6, ContP2		# if($t8 != $t6) then go to ContP2
    addi $t7, $t7, 4 			# $t7 += 4;
    ...
    lb $t6, 0($t7)			# $t6 = Tokens[$t7];
    bne $t6, $t8, ContP2		# if($t8 != $t6) then go to ContP2
    j LoopTok				# else go to LoopTok
...
#only one punct mark
    ...
    li $t8, 44 				# $t8 = 44;
    beq $t6, $t8, PunctIncorrect	# if($t6 == $t8) then go to PunctIncorrect
...
PunctIncorrect:
    ...
    li $s7, 10				# $s7 = 10;
    li $t9, 1				# $t9 = 1;
    j LoopIncorP			# go to LoopIncorP;

        
LoopIncorP:
    beq $s7, $s5, IncorEndP		# if($s7 == $s5) then go to IncorEndP
    beq $t9, $s5, IncorEndP		# if($t9 == $s5) then go to IncorEndP
    beq $zero, $s5, IncorEndP		# if($s5 == 0) then go to IncorEndP
    addi $s2, $s2, 4			# $s2 += 4;
    sw $s4, 0($s2)			# Tokens[$s2] = $s4;
    move $s4, $s5			# $s4 = $s5;
    addi $s2, $s2, 4			# $s2 += 4;
